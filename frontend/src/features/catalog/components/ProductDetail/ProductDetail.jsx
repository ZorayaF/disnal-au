...
const IconCheck = () => (
  <svg className="w-4 h-4 text-disnal-red shrink-0" viewBox="0 0 24 24" fill="none"
...
const IconAward = () => (
  <svg className="w-[22px] h-[22px] text-disnal-red shrink-0" viewBox="0 0 24 24" fill="none"
    stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const IconInfo = () => (
  <svg className="w-4 h-4 shrink-0 text-blue-700" viewBox="0 0 24 24" fill="none"
    stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="12" r="10" />
...
const IconDrop = () => (
  <svg className="w-4 h-4 text-neutral-400 shrink-0" viewBox="0 0 24 24" fill="none"
    stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const ProductDetail = ({ producto, isAuthenticated, userRole }) => {
  ...
  const tieneSpecs = producto?.detallesTecnicos &&
    Object.keys(producto.detallesTecnicos).length > 0;
  const categoriaBadge = producto.presentacion || producto.categoria || "Premium";
    ...
    <div className="w-full max-w-5xl mx-auto bg-white text-[#0b0b0b] rounded-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.08)] overflow-hidden font-sans">
      <div className="grid grid-cols-1 md:grid-cols-[minmax(300px,_1.05fr)_minmax(340px,_1fr)] gap-0 items-start">
        ...
        <div className="p-6 sm:p-10 md:pl-5 lg:pl-8 border-t md:border-t-0 md:border-l border-neutral-200 flex flex-col w-full">
          ...
          <div className="w-10 h-[3px] bg-disnal-red rounded-full mb-5" aria-hidden="true" />
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-5 w-full">
            <div className="flex flex-col gap-2 flex-1 text-sm">
              <div className="flex items-center gap-2.5">
                ...
                <span className={disponible ? "text-emerald-600 font-bold" : "text-disnal-red font-bold"}>
              ...
              {isAuthenticated && (
                <div className="flex items-center gap-2.5">
                  <IconBox />
                  <span className="font-bold text-[#0b0b0b]">Stock General:</span>
                  <span className="text-neutral-600 font-medium">{producto.cantidad} unidades</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-neutral-600 font-medium capitalize">{producto.categoria}</span>
              </div>

              <div className="flex items-center gap-2.5">
                <IconAward />
                <span className="font-bold text-[#0b0b0b]">Marca:</span>
                <span className="text-neutral-600 font-medium capitalize">{producto.marca || "—"}</span>
              </div>
            </div>
            ...
            {isAuthenticated && (
              <div className="flex items-center gap-3 p-[12px_16px] bg-[#f7f7f7] rounded-[10px] min-w-[110px] w-fit shrink-0 self-start sm:self-center">
                <svg className="w-9 h-9 text-disnal-red shrink-0" viewBox="0 0 24 24" fill="none"
          ...
          <div className="mb-4.5">
            ...
            {tieneSpecs ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-2 lg:grid-cols-4 border border-neutral-200 rounded-lg overflow-hidden divide-x sm:divide-x md:divide-x-0 lg:divide-x divide-neutral-200 border-collapse">
                {Object.entries(producto.detallesTecnicos).map(([attr, val]) => (
                  <div key={attr} className="flex flex-col items-start gap-1 p-3.5 bg-white border-neutral-200">
                    <div className="mb-0.5">{getSpecIcon(attr)}</div>
                    <span className="text-[0.72rem] font-medium text-neutral-500 capitalize truncate w-full">{attr}</span>
                    <span className="text-lg font-black text-disnal-red tracking-tight leading-none mt-0.5">{val}</span>
                  </div>
                ))}
              </div>
            ) : (
          ...
          {/* Alerta unidades en carrito */}
          {isAuthenticated && cantidadActual > 0 && (
            <div className="flex items-center gap-2 p-[10px_14px] bg-[#eff6ff] rounded-lg mb-4 text-[0.8rem] text-blue-700" role="alert">
              <IconInfo />
              <span>
                Tienes <strong className="font-bold">{cantidadActual} unidades</strong> agregadas al carrito.
              </span>
            </div>
          )}
            ...
            {isAuthenticated ? (
              !esAdmin && (
                <div className="flex flex-col gap-3 w-full mb-5">
                  ...
                  {/* Stepper de cantidad */}
                  <div className="flex items-center justify-between px-4 py-3 border border-neutral-200 rounded-full bg-[#f7f7f7]">
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        ...
                        disabled={cantidad <= 1}
                        className="w-8 h-8 rounded-full border-2 border-neutral-300 bg-white flex items-center justify-center text-neutral-600 font-black text-lg leading-none transition-all hover:border-disnal-red hover:text-disnal-red disabled:opacity-30 disabled:cursor-not-allowed"
                        aria-label="Reducir cantidad"
                      >
                        −
                      </button>
                      <span className="text-[1.1rem] font-black text-[#0b0b0b] min-w-[24px] text-center tabular-nums">
                        {cantidad}
                      </span>
                      <button
                        type="button"
                        onClick={() => setCantidad((c) => Math.min(producto.cantidad - cantidadActual, c + 1))}
                        disabled={cantidad >= (producto.cantidad - cantidadActual) || limiteAlcanzado}
                        className="w-8 h-8 rounded-full border-2 border-neutral-300 bg-white flex items-center justify-center text-neutral-600 font-black text-lg leading-none transition-all hover:border-disnal-red hover:text-disnal-red disabled:opacity-30 disabled:cursor-not-allowed"
